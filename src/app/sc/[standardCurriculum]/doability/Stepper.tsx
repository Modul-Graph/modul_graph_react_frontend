...
const _DoabilityStepper = ({standardCurriculum}: DoabilityStepperProps) => {
    const [{
        allowNavigation,
        slideNo
    }, dispatcher] = useReducer(reducer, {allowNavigation: STEPS_WITHOUT_NAVIGATION.includes(0), slideNo: 0})
    const [data, setData] = useState<DoabilityResponse>({
        status: "error", message: "Machbarkeitsanalyse fehlgeschlagen"
    })
    ...
    return <Stack justifyContent={"stretch"} alignContent={"stretch"} height={1}>
        ...
        <MobileStepper
                ...
                nextButton={<StepperNextButton allowNavigation={allowNavigation} slideNo={slideNo} maxSlides={NUM_STEPS}
                                               next={dispatcher}/>}
                steps={3}
                activeStep={slideNo}
        />
 ...
 * @param action
 */
const reducer: Reducer<{ allowNavigation: boolean, slideNo: number }, "next"> = ({
                                                                                     slideNo,
                                                                                     allowNavigation
                                                                                 }) => {
...
export type DoabilityStepperProps = {
