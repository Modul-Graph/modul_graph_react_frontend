...
function getStyles(name: string, moduleName: readonly string[], theme: Theme) {
    return {
        fontWeight:
            moduleName.indexOf(name) === -1 ? theme.typography.fontWeightRegular : theme.typography.fontWeightMedium,
    };
}
...
export default function SelectModuleArea({
    ...
    setCompulsoryModule = setCompulsoryModule ?? ((..._) => {});
            ...
            <FormControl sx={{ m: 1, width: 300, mt: 3 }}>
                ...
                >
                    ...
                    {selectionModulArea.map((name) => (
                        <MenuItem key={name} value={name} style={getStyles(name, moduleAreaName ?? [], theme)}>
                            {name}
                        </MenuItem>
                    ))}
                </Select>
...
type ModuleAreaProps = {
    selectionModulArea?: string[];
    moduleAreaName?: string[];
    setModuleAreaName?: (state: string[]) => void;
    compulsoryModule?: boolean;
    setCompulsoryModule?: (state: boolean) => void;
};
