// TODO: Reimplement this component
...
export default function SelectCreditPoints({ creditPointsVal, setCreditPoints }: CreditPointsProps) {
    return (
        <Autocomplete
            ...
            sx={{ m: 1, mt: 3 }}
            freeSolo
            renderTags={(value, getTagProps: (arg0: { index: any }) => JSX.IntrinsicAttributes) =>
                value.map((option, index) => {
                    return <Chip key={index} variant="outlined" label={option} {...getTagProps({ index })} />;
                })
            }
            renderInput={(params: any) => <TextField {...params} label="Credit Points" />}
        />
    );
}
